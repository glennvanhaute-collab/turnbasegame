...
export const HERO_TEMPLATES = {
  ...
  DAEMAR_VAERYN: () => new Hero({
    ...
    lore: 'A pyre-warden of the Ashen Reach, which is a soldier\'s post dressed as a priest\'s. Vaeryn burns what Mordaine buries, and somebody has to stand inside the stone circle while it burns and make certain nothing walks back out — that is the whole of the job. Daemar took the post at nineteen, which is young for it, and was already white-haired by then, which is not young for anything. The Reach raises them in that smoke and the colour goes early. Mordaine wardens have called his rite heresy to his face since before he could grow a beard and have never once told him to stop performing it, and he has drawn his own conclusions about what that means. He goes where his liege sends him. He does not pretend to agree.',
  }),
